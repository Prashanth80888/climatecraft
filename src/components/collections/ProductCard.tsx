...
export function ProductCard({ product, total }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className="group relative block h-full overflow-hidden rounded-[22px] border border-[#0B3F42]/20 bg-[#F4F7F5] transition-[border-color,box-shadow] duration-300 ease-out will-change-transform hover:border-[#159FA3]/50 hover:shadow-[0_32px_64px_-30px_rgba(6,61,60,0.35)]"
    >
      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#E8EFEC]">
        ...
        {/* Bottom Content Area */}
        <div className="absolute inset-x-0 bottom-0 z-20 p-5 transition-transform duration-500 ease-out group-hover:-translate-y-1 sm:p-6">
          <div className="flex items-start justify-between gap-3">
            <h4 className="font-display text-lg font-semibold leading-snug text-white drop-shadow-md sm:text-xl group-hover:text-gold-200">
              {product.name}
            </h4>
          ...
          <p className="mt-2 max-w-xs text-[13px] leading-relaxed text-white/90 drop-shadow-sm line-clamp-2">
            {product.teaser}
          </p>
