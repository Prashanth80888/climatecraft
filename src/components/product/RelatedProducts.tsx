...
function RelatedCard({ product }: { product: HomeProduct }) {
  ...
  const onMouseMove = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    card.style.setProperty(
      '--tilt-x',
      `${((0.5 - py) * MAX_TILT * 2).toFixed(2)}deg`,
    )

    card.style.setProperty(
      '--tilt-y',
      `${((px - 0.5) * MAX_TILT * 2).toFixed(2)}deg`,
    )
  }
      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#E8EFEC]">
        {product.imageCount > 0 ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-gradient-to-br from-canvas-aqua to-canvas text-center">
            <Camera
              className="h-6 w-6 text-cream-200"
              strokeWidth={1.5}
            />
        ...
        <div className="absolute inset-x-0 bottom-0 h-[48%] bg-gradient-to-t from-black/65 via-black/30 to-transparent" />

        {/* Product number */}
        <span
          className="absolute left-4 top-4 font-display text-xs italic tabular-nums text-white drop-shadow-[0_2px_5px_rgba(0,0,0,0.65)]"
        >
          {String(product.number).padStart(2, '0')} /{' '}
          {String(HOME_PRODUCTS.length).padStart(2, '0')}
        </span>
        ...
        <div className="absolute inset-x-0 bottom-0 p-5 transition-transform duration-500 ease-out group-hover:-translate-y-1">
          <div className="flex items-start justify-between gap-3">
            <h4
              className="font-display text-lg font-medium text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.75)]"
            >
              {product.name}
            </h4>

            <ArrowUpRight
              className="mt-1 h-5 w-5 flex-none -translate-x-1 translate-y-1 text-gold-400 opacity-0 drop-shadow-[0_2px_5px_rgba(0,0,0,0.7)] transition-all duration-500 group-hover:translate-x-0 group-hover:translate-y-0 group-hover:opacity-100"
            />
          </div>

          <p
            className="mt-1 text-[12px] font-medium uppercase tracking-[0.14em] text-white/95 drop-shadow-[0_2px_5px_rgba(0,0,0,0.75)]"
          >
            {product.category}
          </p>
        </div>
