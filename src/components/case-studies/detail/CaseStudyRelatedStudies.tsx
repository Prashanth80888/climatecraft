          ...
          {caseStudies.map((cs) => (
            <RevealItem key={cs.slug}>
              <Link
                ...
                className="group relative block overflow-hidden rounded-[20px] border border-[#0B3F42]/[0.15] bg-[#F4F7F5] transition-all duration-300 hover:border-[#159FA3]/35 hover:shadow-[0_24px_56px_-24px_rgba(6,61,60,0.22)]"
              >
                <div className="relative aspect-[16/10] w-full overflow-hidden bg-[#E8EFEC]">
                  ...
                  <span className="absolute left-4 top-4 rounded-full border border-ink-900/[0.14] bg-white/50 px-3 py-1 text-[10px] font-medium uppercase tracking-widest text-cream-100 backdrop-blur-md">
                    {cs.category}
                  </span>
                  ...
                  <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-5 transition-transform duration-500 ease-out group-hover:-translate-y-1">
                    <div>
                      ...
                      <p className="mt-1 max-w-xs text-[12.5px] leading-relaxed text-cream-200">{cs.summary}</p>
                    </div>
